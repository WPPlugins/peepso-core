...
function dropdown_toggle_click(e)
{
	...
	var menu = el.siblings(".ps-dropdown-menu");

	jQuery(".ps-dropdown-menu").not(menu).css("display", "none");
	...
	jQuery(document).on("mouseup.peepso-dropdown", function(e) {
		if (!menu.is(e.target) && !el.is(e.target) && 0 === menu.has(e.target).length && (0 === el.has(e.target).length)) {
			el.removeClass("open");
			menu.hide();
...
function dropdown_toggle_mouseenter(e)
{
	...
	if (winsize !== 'small' && winsize !== 'xsmall') {
		...
		jQuery("#peepso-wrap .ps-dropdown-toggle").removeClass("open").siblings(".ps-dropdown-menu").css("display", "none");
	}
}
...
function dropdown_toggle_a_click(e)
{
	...
	if (hidden && hidden.length) {
		hidden.val(el.data("option-value")).triggerHandler("change");
	}
...
jQuery(document).ready(function($) {
	...
	$(document).on("click", ".ps-dropdown-toggle", dropdown_toggle_click)
...
/*	$("#peepso-wrap .ps-dropdown-toggle").click( function(e) {
		...
		var icon = ct.find(".ps-dropdown-toggle").find("i");
		var span = ct.prev(".ps-share-privacy");
		jQuery(".ps-dropdown-menu").css("display", "none");
		var menu = el.siblings(".ps-dropdown-menu");
		menu.css("display", "block");
		...
		// turn off any drop-downs when first entering a drop-down
		jQuery("#peepso-wrap .ps-dropdown-toggle").siblings(".ps-dropdown-menu").css("display", "none");
	}); */

	// hides dropdown menu after menu item is clicked
	$(document).on("click", ".ps-dropdown-menu a", dropdown_toggle_a_click);
/*	$("#peepso-wrap .ps-dropdown-menu a").click(function(e) {
