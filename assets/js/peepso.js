...
//			.always(function(e) { console.log("== always") });
	...
	getLinkColor: function( opacity ) {
		...
		if ( this._linkColor ) {
			...
			$dummy.remove();
			if ( parts = color.match( /^rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*(\d+(?:\.\d+)?))?\)$/ )) {
		...
		if ( typeof color !== 'string' ) {
			color = 'rgba(' + color[0] + ',' + color[1] + ',' + color[2] + ',' + ( opacity || color[3] ) + ')';
		}
...
jQuery(document).ready(function() {
	 ...
	 * @param {boolean} reload Reload current page
	 */
	jQuery(window).on("peepso_auth_required", function(e, reload) {
		...
		pswindow.show("Please login to continue", peepsodata.login_dialog);
		...
		if (reload) {
			jQuery('input[name=redirect_to]').val( window.location );
		}
	});
	...
	var className = '' + ( peepso.isMobile() ? ' ps-mobile' : '' ) + ( peepso.isTouch() ? ' ps-touch' : '' );
	if ( className !== '' ) {
...
ps_login.form_submit = function(e)
{
	...
	var data = {
		...
		security: form.parent().find("form[name=loginform]").find("#security").val(),
...
jQuery(function( $ ) {
	$('.ps-toolbar__toggle').toggle(function () {
		$("#ps-main-nav").addClass("open");
	}, function () {
		$("#ps-main-nav").removeClass("open");
	});
});
