...
PsWindow.prototype.hide = function() {
	...
	if (typeof ps_observer !== 'undefined') {
		ps_observer.do_action('pswindow_before_close', $dialog );
	}

	$dialog.hide();
	$dialog.find('#cwin_logo').empty();
	$dialog.find('#cWindowContent').empty();
	$dialog.find('#cWindowAction').remove();

	this.is_visible = false;
	$dialog.trigger('pswindow.hidden');

	// fires onclose actions
	if (typeof ps_observer !== 'undefined') {
		ps_observer.apply_filters('pswindow_close');
		ps_observer.do_action('pswindow_close', $dialog );
	}
