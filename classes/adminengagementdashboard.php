...
class PeepSoAdminEngagementDashboard extends PeepSoAjaxCallback
{/*
...
//	stats[]=likes
		...
		$stats = $this->_input->val('stats', 'peepso-comment,peepso-post,likes');
		$stats = explode(',', $stats);
	...
	public function get_stats($type, &$resp)
	{
		...
		$module_id = $this->_input->val('module_id');

		add_filter('peepso_activity_date_range_where', array(&$this, 'get_date_range_where'));
		$range = apply_filters('peepso_activity_date_range_where', $this->_input->val('date_range'));
