...
*/
	...
	public function get_orientation()
	{
		if (NULL === $this->_filename)
			return ($this->_set_error(__('no file given', 'peepso-core') . ' (' . var_export($this->_filename, TRUE) . ')'));

		$this->_fh = fopen($this->_filename, 'r');
		if (FALSE === $this->_fh)
			return ($this->_set_error(__('unable to open file', 'peepso-core')));
		...
		if (0xFF != $exif_data[0] ||
			...
			0xFF != $exif_data[2] ||
			0xE1 != $exif_data[3])
			return ($this->_set_error(__('JPG header not found', 'peepso-core')));
...
//echo 'length: ', $length, PHP_EOL;
		...
		if (0x45 != $exif_data[0] ||
			...
			0 != $exif_data[4] ||
			0 != $exif_data[5])
			return ($this->_set_error(__('Exif data not found ' . var_export($exif_data, TRUE), 'peepso-core')));
		...
		if ($length < 12)
			return ($this->_set_error(__('bad IFD entry length', 'peepso-core'))); // length of an IFD entry
		...
		if (0x49 == $exif_data[0] && 0x49 == $exif_data[1])
			...
			return ($this->_set_error(__('unable to determine byte order', 'peepso-core')));

		// check Tag mark
		if ($is_motorola) {
			if (0 != $exif_data[2])
				return ($this->_set_error(__('invalid tag mark', 'peepso-core')));
			if (0x2A != $exif_data[3])
				return ($this->_set_error(__('unrecognized Tag mark: 0x', 'peepso-core') . dechex($exif_data[3])));
		} else {
			if (0 != $exif_data[3])
				return ($this->_set_error(__('invalid tag mark', 'peepso-core')));
			if (0x2A != $exif_data[2])
...
//echo 'found proper tag mark', PHP_EOL;
		...
		if ($is_motorola) {
			if (0 != $exif_data[4])
				return ($this->_set_error(__('invalid IFD offset', 'peepso-core')));
			if (0 != $exif_data[5])
				return ($this->_set_error(__('invalid IFD offset', 'peepso-core')));
//echo '0x', dechex($exif_data[6]), ' 0x', dechex($exif_data[7]), PHP_EOL;
		...
		if ($offset > $length - 2)
			return ($this->_set_error(__('invalid offset', 'peepso-core')));
//echo 'offset: ', $offset, PHP_EOL;
		...
		if (0 === $number_of_tags)
			return ($this->_set_error (__('invalid number of tags: ', 'peepso-core') . $number_of_tags));
//echo 'number of tags: ', $number_of_tags, PHP_EOL;
		...
		for (;;) {
			// check end of data segment
			if ($offset > $length - 12)
				return ($this->_set_error(__('exceeded end of data segment', 'peepso-core')));
			// get Tag number
			if ($is_motorola) {
...
//echo 'tagnum: ', $tagnum, PHP_EOL;
			...
			if (0 == --$number_of_tags)
				return ($this->_set_error(__('number of bytes is zero', 'peepso-core')));
			$offset += 12;
		}
		...
		if ($is_motorola) {
			if (0 != $exif_data[$offset + 8])
				return ($this->_set_error(__('orientation value is 0', 'peepso-core')));
			$set_flag = $exif_data[$offset + 9];
		} else {
			if (0 != $exif_data[$offset + 9])
				return ($this->_set_error(__('orientagion value is 0', 'peepso-core')));
			$set_flag = $exif_data[$offset + 8];
		}
//echo 'set_flag: ', $set_flag, PHP_EOL;
		if ($set_flag > 8)
			return ($this->_set_error(__('invalid orientation tag:', 'peepso-core') . $set_flag));
	...
	private function _read1()
	{
		$ch = fread($this->_fh, 1);
		if (FALSE === $ch)
			return ($this->_set_error(__('premature EOF', 'peepso-core')));
		return ($ch);
	}
